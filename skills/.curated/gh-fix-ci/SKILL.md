...
metadata:
  ...
  description-zh: 使用 gh 查看 GitHub PR checks，拉取失败的 GitHub Actions 日志，汇总失败上下文，然后给出修复计划并在用户批准后实施。用于用户想调试或修复 GitHub Actions 上的 CI/CD 失败；外部检查（如 Buildkite）只报告详情链接并标记为不在范围内。
---
